...
export class CategoryChartService {
    ...
    let option = {
      ...
      "tooltip": {
        ...
        "formatter": function () {
          ...
          let toolTipTemplate = ["Phân loại", "Doanh thu", "So " + unit.toLowerCase() + " trước", unit + " báo cáo"];
          ...
          "events": {
            ...
            click: function (e) {
              ...
              for (let i = 0; i < series.length; i++) {
                ...
                for (let j = 0; j < sData.length; j++) {
                  ...
                  if (dCate !== category) {
                    if (dColor.startsWith("rgba") && color.startsWith("#")) {
                      ...
                      categoryService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter("", cate, "", "", ""), areaService, categoryService, deptService, gametypeService, "category-chart");
                    } else {
                ...
                areaService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter("", cate, "", "", ""), areaService, categoryService, deptService, gametypeService, "area-chart");
                deptService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter("", cate, "", "", ""), areaService, categoryService, deptService, gametypeService, "dept-chart");
                gametypeService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter("", cate, "", "", ""), areaService, categoryService, deptService, gametypeService, "gametype-chart");
              }
            }
          }
        },
  ...
  loadStaticChart(rootUrl: string, timeUnit: string, timeSelect: string, filter: string, areaService: any, cateService: any, deptService: any, gametypeService: any, id: string) {
    let url = rootUrl + "metrics/revenue/category?time_unit=" + timeUnit + "&time_select=" + timeSelect + filter;
    console.log(url);
