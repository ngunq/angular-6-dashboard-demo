...
export class DeptPuChartService {
  hidden: boolean = true;
    ...
    let option = {
      ...
      "tooltip": {
        ...
        "formatter": function () {
          ...
          //     break;
          // }
          // let toolTipTemplate = ["Department", "Paying user", "Compare last " + unit, "Report " + unit];
          switch (timeUnit) {
            case "month":
              unit = "Tháng";
              break;
          ...
          let toolTipTemplate = ["Phòng ban", "Số lượng", "So " + unit.toLowerCase() + " trước", unit + " báo cáo"];
          ...
          "events": {
            ...
            click: function (e) {
              ...
              for (let i = 0; i < series.length; i++) {
                ...
                for (let j = 0; j < sData.length; j++) {
                  ...
                  if (dCate !== category) {
                    if (dColor.startsWith("rgba") && color.startsWith("#")) {
                      ...
                      deptService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter("", "", dept, "", ""), areaService, categoryService, deptService, gametypeService, "dept-pu-chart");
                    } else {
                ...
                categoryService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter("", "", dept, "", ""), areaService, categoryService, deptService, gametypeService, "category-pu-chart");
                areaService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter("", "", dept, "", ""), areaService, categoryService, deptService, gametypeService, "area-pu-chart");
                gametypeService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter("", "", dept, "", ""), areaService, categoryService, deptService, gametypeService, "gametype-pu-chart");
              }
            }
          }
        },
  ...
  loadStaticChart(rootUrl: string, timeUnit: string, timeSelect: string, filter: string, areaService: any, cateService: any, deptService: any, gametypeService: any, id: string) {
    let url = rootUrl + "metrics/pu/dept?time_unit=" + timeUnit + "&time_select=" + timeSelect + filter;
    console.log(url);
