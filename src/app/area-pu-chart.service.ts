...
export class AreaPuChartService {
  hidden: boolean = true;
          ...
          "events": {
            ...
            click: function (e) {
              ...
              for (let i = 0; i < series.length; i++) {
                ...
                for (let j = 0; j < sData.length; j++) {
                  ...
                  if (dCate !== category) {
                    if (dColor.startsWith("rgba") && color.startsWith("#")) {
                      ...
                      area = "";
                      // areaService.hidden = true;
                      areaService.setHidden(true, false, true);
                      areaService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter(area, "", "", "", ""), areaService, categoryService, deptService, gametypeService, "area-chart");
                    } else {
                ...
                categoryService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter(area, "", "", "", ""), areaService, categoryService, deptService, gametypeService, "category-chart");
                deptService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter(area, "", "", "", ""), areaService, categoryService, deptService, gametypeService, "dept-chart");
                gametypeService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter(area, "", "", "", ""), areaService, categoryService, deptService, gametypeService, "gametype-chart");
              }
            }
          }
        }
    ...
    let option = {
      ...
      tooltip: {
        ...
        formatter: function () {
          ...
          //     break;
          // }
          // let toolTipTemplate = ["Market", "Paying user", "Compare last " + unit, "Report " + unit];
          switch (timeUnit) {
            case "month":
              unit = "Tháng";
              break;
          ...
          let toolTipTemplate = ["Khu vực", "Số lượng", "So " + unit.toLowerCase() + " trước", unit + " báo cáo"];
        ...
        series: {
          events: {
            click: function (e) {
              ...
              for (let i = 0; i < pData.length; i++) {
                ...
                if (pData[i].name !== pName) {
                  if (pData[i].color.startsWith("rgba") && pColor.startsWith("#")) {
                    ...
                    areaService.setHidden(true, false, true);
                    areaService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter(area, "", "", "", ""), areaService, categoryService, deptService, gametypeService, "area-pu-chart");
                  } else {
              ...
              categoryService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter(area, "", "", "", ""), areaService, categoryService, deptService, gametypeService, "category-pu-chart");
              deptService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter(area, "", "", "", ""), areaService, categoryService, deptService, gametypeService, "dept-pu-chart");
              gametypeService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter(area, "", "", "", ""), areaService, categoryService, deptService, gametypeService, "gametype-pu-chart");
            }
          }
        }
      },
  ...
  static getChart(id: string) {
    for (let i = 0; i < Highcharts.charts.length; i++) {
      if (Highcharts.charts[i].renderTo.id === id) {
        return i;
      }
    }
    return -1;
  ...
  loadStaticChart(rootUrl: string, timeUnit: string, timeSelect: string, filter: string, areaService: any, cateService: any, deptService: any, gametypeService: any, id: string) {
    let url = rootUrl + "metrics/pu/area?time_unit=" + timeUnit + "&time_select=" + timeSelect + filter;
    console.log(url);
    this.http.get(url).subscribe((response: any) => {
      if (response.toString() !== "-1") {
        ...
        this.drawPieChart(id, "PU" + name, data, tooltip, rootUrl, timeUnit, timeSelect, areaService, cateService, deptService, gametypeService);
      } else {
