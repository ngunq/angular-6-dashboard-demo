...
export class DeptChartService {
    ...
    let option = {
      ...
      "tooltip": {
        ...
        "formatter": function () {
          ...
          let toolTipTemplate = ["Phòng ban", "Doanh thu", "So " + unit.toLowerCase() + " trước", unit + " báo cáo"];
          ...
          "events": {
            ...
            click: function (e) {
              ...
              for (let i = 0; i < series.length; i++) {
                ...
                for (let j = 0; j < sData.length; j++) {
                  ...
                  if (dCate !== category) {
                    if (dColor.startsWith("rgba") && color.startsWith("#")) {
                      ...
                      deptService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter("", "", dept, "", ""), areaService, categoryService, deptService, gametypeService, "dept-chart");
                    } else {
                ...
                categoryService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter("", "", dept, "", ""), areaService, categoryService, deptService, gametypeService, "category-chart");
                areaService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter("", "", dept, "", ""), areaService, categoryService, deptService, gametypeService, "area-chart");
                gametypeService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter("", "", dept, "", ""), areaService, categoryService, deptService, gametypeService, "gametype-chart");
              }
            }
          }
        },
  ...
  loadStaticChart(rootUrl: string, timeUnit: string, timeSelect: string, filter: string, areaService: any, cateService: any, deptService: any, gametypeService: any, id: string) {
    let url = rootUrl + "metrics/revenue/dept?time_unit=" + timeUnit + "&time_select=" + timeSelect + filter;
    console.log(url);
    this.http.get(url).subscribe((response: any) => {
      if (response.toString() !== "-1") {
        ...
        this.drawChart(id, "Doanh thu " + name, categories, data, tooltip, rootUrl, timeUnit, timeSelect, areaService, cateService, deptService, gametypeService);
      } else {
