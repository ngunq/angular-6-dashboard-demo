...
export class GametypePuChartService {
  hidden: boolean = true;
    ...
    let option = {
      ...
      tooltip: {
        ...
        formatter: function () {
          ...
          //     break;
          // }
          // let toolTipTemplate = ["Game type", "Paying user", "Compare last " + unit, "Report " + unit];
          switch (timeUnit) {
            case "month":
              unit = "Tháng";
              break;
          ...
          let toolTipTemplate = ["Thể loại", "Số lượng", "So " + unit.toLowerCase() + " trước", unit + " báo cáo"];
        ...
        series: {
          events: {
            click: function (e) {
              ...
              for (let i = 0; i < pData.length; i++) {
                ...
                if (pData[i].name !== pName) {
                  if (pData[i].color.startsWith("rgba") && pColor.startsWith("#")) {
                    ...
                    gametype = "";
                    gametypeService.setHidden(true, false, true);
                    gametypeService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter("", "", "", gametype, ""), areaService, categoryService, deptService, gametypeService, "gametype-pu-chart");
                  } else {
              ...
              categoryService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter("", "", "", gametype, ""), areaService, categoryService, deptService, gametypeService, "category-pu-chart");
              deptService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter("", "", "", gametype, ""), areaService, categoryService, deptService, gametypeService, "dept-pu-chart");
              areaService.loadStaticChart(rootUrl, timeUnit, timeSelect, AppScript.buildFilter("", "", "", gametype, ""), areaService, categoryService, deptService, gametypeService, "area-pu-chart");
            }
          }
        }
      },
  ...
  loadStaticChart(rootUrl: string, timeUnit: string, timeSelect: string, filter: string, areaService: any, cateService: any, deptService: any, gametypeService: any, id: string) {
    let url = rootUrl + "metrics/pu/gametype?time_unit=" + timeUnit + "&time_select=" + timeSelect + filter;
    console.log(url);
